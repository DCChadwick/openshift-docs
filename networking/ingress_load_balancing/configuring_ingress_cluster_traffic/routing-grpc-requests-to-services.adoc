...
While standard `GRPCRoute` configurations share many similarities with `HTTPRoute` resources, the {product-title} implementation of `GRPCRoute` has strict schema differences and specific limitations, such as a reduced match limit per rule and the removal of experimental fields.
...
* Understand `GRPCRoute` schema differences
...
include::modules/understanding-grpcroute-schema-differences.adoc[leveloffset=+1]