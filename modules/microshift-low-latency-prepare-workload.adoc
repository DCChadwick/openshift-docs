...
----
+
`cpu-load-balancing.crio.io`:: Disables the CPU load balancing for the pod.
`irq-load-balancing.crio.io`:: Opts the pod out of interrupt handling (IRQ).
`cpu-quota.crio.io`:: Disables the CPU completely fair scheduler (CFS) quota at the pod run time.
`cpu-c-states.crio.io`:: Enables or disables C-states for each CPU. Set the value to `disable` to provide the best performance for a high-priority pod.
`cpu-freq-governor.crio.io`:: Sets the `cpufreq` governor for each CPU. The `performance` governor is recommended for high-priority workloads.
`runtimeClassName`:: The `runtimeClassName` must match the name of the performance profile configured in the node. For example, `microshift-low-latency`.
+
[NOTE]
====
